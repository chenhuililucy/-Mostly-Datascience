...
class Photo:
    def __init__(self, i, line):
        ...
        self.tags = set(words[2:]) #convert tags into set 
...
class Slide:
    def __init__(self, photos):
        # initialize using the first photo of the list given 
        ...
        if len(photos) > 1:
            self.tags = self.tags | photos[1].tags # find intersection between tags in photo 1 and 2 
        self.salt = random.getrandbits(salt_bits)

    def interest(self, other):
        # iteratively def self and other and find union and intersection between self and other 
...
class Instance:
    def __init__(self, infile):
        ...
        N = int(next(infile)) # next() is an iterator, called repeatedly; the method returns the next
        # input or raises StopIteration when EOF is hit 
        # cannot combine next() method with other method such as readline
        self.photos = []
...
def main():
    ...
    while True:
        for infile, infile_solution in itertools.zip_longest(namespace.instance, namespace.solution):
            ...
            infile.seek(0) #sets the file's current position at the offset 
            # the whence argument is optinoal and defaults to 0, which means 
            # absolute file positioning, other values are 1 which means seek 
            # relative to the current position and 2 means seek relative to the 
            # file's end 
