...
class Solution:
    ...
    def __init__(self,noofbooks=None,signupdays=None,shippingdays=None,IDsall=None):
        pass
    ...
    def read(self): 
        x="/Users/lucy/Desktop/f_libraries_of_the_world.txt"
        ...
        with open(x,"r") as file: 
            ...
            B, L, D= [int(x) for x in item] 
            Bookscores=[int(x) for x in (filedata[1].rstrip().split(" "))]
            
            for i in range(0,count,2):
                books,signups,shippings = filedata[2+i].rstrip().split(" ")
                noofbooks.append(books)

                signupdays.append(signups)
                shippingdays.append(shippings)
        ...
        return Solution(noofbooks,signupdays,shippingdays,IDsall)



class Library(Solution):
    def __init__(self,noofbooks,signupdays,shippingdays): 
        
        self.noofbooks=noofbooks
        self.signupdays=signupdays
        self.shippingdays=shippingdays
    
    def sortsignupdays(self): 

        signupheap=heapq.heapify(self.signupdays)

        sortedsignupdays=[]
        for i in range(len(self.signupdays)):  
            sortedsignupdays.append(heapq.heappop(self.signupdaysall))
        print(sortedsignupdays)

    def align(self,sortsignupdays): 
        indexlist=[]
